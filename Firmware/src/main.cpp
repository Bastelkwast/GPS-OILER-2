...
char firmware_Vers[] = "Rev.2.7"; // Aktuelle Firmwareversion
...
// Benutzerdifinierte Variablen. Werden später mit den Werten aus dem Speicher überschrieben
...
boolean Init_Oil = true;           // Wird für die erste Ölung benötigt. Ölt wenn die Min.Geschwindigkeit überschritten wird
...
boolean oilen = true;              // Wird gesetzt wenn die nächste Ölung ansteht. Steht auf True damit ein Initialer Ölungspumpstoss ausgelöst wird
...
byte pump_anzahl_soll = 1;         // Aktuelle Anzahl der Pumpimpulse. Steht auf 1 für die Initiale Ölung
...
void make_HTML01()
{
  ...
  strcat(HTML_String, "<p>Distanz bis zur n&auml;chsten &Ouml;lung in Meter, Impulse pro &Ouml;lung, Mindestgeschwindigkeit zum &Ouml;len.</p>");
...
void update_Display()
{
  ...
  display.drawRoundRect(1, 27, 75, 25, 4, WHITE);

  // Zeichnet den Rahmen für die Geschwindigkeit
  display.drawRoundRect(79, 0, 48, 25, 4, WHITE);

  // Zeichnet den Rahmen für die Richtung
  display.drawRoundRect(79, 27, 48, 25, 4, WHITE);
...
void read_char_array(int address, int length, int i2c_address, char *rcvData)
{
  ...
  {
    // Begint die Übertragung zum I2C EEPROM
    Wire.beginTransmission(i2c_address);

    // Überträgt die Daten zum Speicher
